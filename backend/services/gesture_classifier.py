...
Loads the trained gesture recognition model (MobileNetV3Large, 5 classes)
...
class GestureClassifier:
    ...
    # Gesture labels
    GESTURES = ['left_swipe', 'right_swipe',
                'stop', 'thumbs_down', 'thumbs_up']
    ...
    def _load_models(self):
        ...
        if classes_path.exists():
            ...
            print(
                f"[GestureClassifier] Loaded {len(self.classes)} gesture classes")
        else:
        ...
        if model_path.exists():
            try:
                ...
                print(
                    f"[GestureClassifier] ✅ Loaded optimized model (99.95% accuracy)")
            except Exception as e:
                print(
                    f"[GestureClassifier] ⚠️  Failed to load model: {str(e)[:100]}")
        else:
            print(
                f"[GestureClassifier] ⚠️  Optimized model not found: {model_path}")
            print(
                "[GestureClassifier] Run: python backend/scripts/train_leap_gesture.py")
    ...
    def _preprocess_image(self, img: np.ndarray) -> np.ndarray:
        ...
        img_resized = cv2.resize(
            img_rgb, self.img_size, interpolation=cv2.INTER_LANCZOS4)
    ...
    def _ensemble_predict(self, img_tensor: np.ndarray) -> Tuple[str, float, Dict[str, float]]:
        ...
        all_probs = {self.classes[i]: float(
            softmax_pred[i]) for i in range(len(self.classes))}
